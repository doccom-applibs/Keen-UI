  ...
  <div class="ui-autocomplete" :class="classes">
    ...
    <div class="ui-autocomplete__content">
      <label class="ui-autocomplete__label">
        <div class="ui-autocomplete__label-text" :class="labelClasses" v-if="label || $slots.default">
          <slot>{{ label }}</slot>
        </div>
        ...
        <input autocomplete="off" class="ui-autocomplete__input" ref="input" :disabled="disabled" :name="name" :placeholder="hasFloatingLabel ? null : placeholder" :readonly="readonly ? readonly : null" :value="value" :maxlength="enforceMaxlength ? maxlength : null" @blur="onBlur" @change="onChange" @focus="onFocus" @input="updateValue($event.target.value)" @keydown.down.prevent="highlightSuggestion(highlightedIndex + 1)" @keydown.enter="selectHighlighted(highlightedIndex, $event)" @keydown.esc="closeDropdown" @keydown.tab="selectHighlighted(highlightedIndex, $event)" @keydown.up.prevent="highlightSuggestion(highlightedIndex - 1)" v-autofocus="autofocus">

        <ul class="ui-autocomplete__suggestions" v-show="showDropdown">
          <ui-autocomplete-suggestion ref="suggestions" :highlighted="highlightedIndex === index" :key="index" :keys="keys" :suggestion="suggestion" :type="type" @click.native="selectSuggestion(suggestion)" v-for="(suggestion, index) in matchingSuggestions">
            <slot name="suggestion" :highlighted="highlightedIndex === index" :index="index" :suggestion="suggestion"></slot>
          </ui-autocomplete-suggestion>
        </ul>
      </label>

      <div class="ui-autocomplete__feedback" v-if="hasFeedback || maxlength">
  ...
  export default {
    ...
    methods: {
      ...
      selectSuggestion(suggestion) {
        ...
        if (this.append) {
          value +=
            this.appendDelimiter + (suggestion[this.keys.value] || suggestion);
        } else {
