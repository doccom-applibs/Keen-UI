<template>
  <div class="ui-datepicker" :class="classes">
    ...
    <div class="ui-datepicker__content">
      <div class="ui-datepicker__label" ref="label" :tabindex="disabled ? null : '0'" @click="onClick" @focus="onFocus" @keydown.enter.prevent="openPicker" @keydown.space.prevent="openPicker" @keydown.tab="onBlur">
        <div class="ui-datepicker__label-text" :class="labelClasses" v-if="label || $slots.default">
          <slot>{{ label }}</slot>
        </div>
    ...
    <ui-modal ref="modal" remove-header @close="onPickerClose" @open="onPickerOpen" v-if="usesModal && !disabled">
      <ui-calendar :color="color" :date-filter="dateFilter" :lang="lang" :max-date="maxDate" :min-date="minDate" :orientation="orientation" :value="value" @date-select="onDateSelect" :init-with-year-picker="initWithYearPicker">
        <div class="ui-datepicker__modal-buttons" slot="footer">
          <ui-button type="secondary" :color="color" @click="$refs.modal.close()">{{ okButtonText }}</ui-button>

          <ui-button type="secondary" :color="color" @click="onPickerCancel">{{ cancelButtonText }}</ui-button>
        </div>
      </ui-calendar>
    </ui-modal>

    <ui-popover contain-focus ref="popover" trigger="label" :appendTo="appendPopoverTo" @close="onPickerClose" @open="onPickerOpen" v-if="usesPopover && !disabled">
      <ui-calendar :color="color" :date-filter="dateFilter" :lang="lang" :max-date="maxDate" :min-date="minDate" :orientation="orientation" :value="value" @date-select="onDateSelect" :init-with-year-picker="initWithYearPicker"></ui-calendar>
    </ui-popover>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      hasFeedback() {
        return (
          Boolean(this.help) || Boolean(this.error) || Boolean(this.$slots.error)
        );
      },
      ...
      showHelp() {
        return (
          !this.showError && (Boolean(this.help) || Boolean(this.$slots.help))
        );
      },
